...
{
    public partial class frmProductMenu_edit : Form
    {
        void init()
        {
            ...
            txt_name_persian.Text = selected_item.NamePersian;
            txt_name_english.Text = selected_item.NameEnglish;
            txt_calorie.Text = selected_item.Calories.ToString();
            txt_price.Text = selected_item.Price.ToString();
            check_enable.Checked = (bool)selected_item.Enable;
            cmb_category.SelectedItem = selected_item.Category;
            try
            {
                pic_product.Image = Timage.byteArrayToImage(selected_item.Image.ToArray());
            }
            catch
            {
            ...
            memo_info_persian.Text = selected_item.InfoPersian;
            memo_info_english.Text = selected_item.InfoEnglish;

           
            if (selected_item.Image != null)
                lbl_info.Text = ((float)selected_item.Image.Length / 1000).ToString() + " KB";

        }
        Productd selected_item;
        ...
        private void frmProductMenu_edit_Load(object sender, EventArgs e)
        {
            ...
            try
            {
                if (food_price != "" && food_name_english != "" && food_name_persian != "" && food_info_farsi != "" && food_info_english != "")
                {
                    ...
                    for (int i = 0; i < Statics.list_products.Count; i++)
                    {
                        ...
                        if (p.NamePersian == food_name_persian && selected_item.NamePersian != food_name_persian)
                        {
                            throw new Exception("نام کالای فارسی مورد نظر قبلا برای کالای دیگری انتخاب شده است");
                        }
                        if (p.NameEnglish == food_name_english && selected_item.NamePersian != food_name_persian)
                        {
                    ...
                    bool result1 = DBMS_Linq.update_product(selected_item, ToEdit_item);
                    ...
                    if (result1 )
                        MBB = new MBOX("محصول با موفقيت ویرایش شد", "سیستم...", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    else
                        MBB = new MBOX("خطا در ویرایش محصول", "سیستم...", MessageBoxButtons.OK, MessageBoxIcon.Error);
